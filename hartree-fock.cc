...
Matrix compute_2body_fock(const std::vector<libint2::Shell>& shells,
                                 const Matrix& D);
Matrix compute_2body_exchange_simple(const std::vector<libint2::Shell>& shells,
                                     const Matrix& D);
Matrix compute_2body_Coulomb_simple(const std::vector<libint2::Shell>& shells,
                                    const Matrix& D);
...
//    cout << D << endl;
    ...
    auto K = compute_2body_exchange_simple(shells, D);
    cout << "\n\tExchange Matrix:\n";
    cout << K << endl;

    // compute exchange contribution to HF energy
//    Matrix D_final = Coef * Coef.transpose();
//    cout << "\n\t Test Density Matrix:\n";
//    cout << D_final << endl;
    auto e_K = 0.0;
    for (auto i = 0; i < nao; i++)
      for (auto j = 0; j < nao; j++)
   	    e_K += D(i,j) * K(i,j);
    printf("** Exchange contribution to HF energy = %20.12f\n", e_K);
    printf("** Electronic HF energy = %20.12f\n", ehf);

    auto J = compute_2body_Coulomb_simple(shells, D);
    cout << "\n\tCoulomb Matrix:\n";
    cout << J << endl;
